...
<img width="1408" height="768" alt="image" src="https://github.com/user-attachments/assets/e53c1505-6ca8-4503-9a5d-bf26f5a6caf3" />
...
```javascript
// ❌ 문제가 된 코드 (index.html - 기존 도서 카드 렌더링)
// 사용자 데이터(book.title 등)를 템플릿 리터럴로 innerHTML에 직접 삽입
// book.title = "<img src=x onerror=alert('XSS')>" 같은 값이 들어오면 그대로 실행됨
books.forEach(book => {
    grid.innerHTML += `
        <div class="book-card">
            <h3>${book.title}</h3>
...
```javascript
// ✅ 개선된 코드 (index.html - 수정 후)
books.forEach(book => {
    const card = document.createElement('div');
    card.className = 'book-card';
 
    const title = document.createElement('h3');
    title.textContent = book.title; // 어떤 값이 들어와도 텍스트로만 처리됨
 
    const info = document.createElement('p');
    info.textContent = `${book.author} | ${book.publisher}`;
...
```javascript
// ❌ 문제가 된 코드 (my-reviews.html - 기존 리뷰 목록 렌더링)
// 1. 루프 안에서 매번 getElementById() 호출 → 불필요한 DOM 탐색 반복
// 2. innerHTML += 패턴 → 기존 HTML 전체를 매번 파싱하고 재렌더링
reviews.forEach(review => {
    ...
    `;
    document.getElementById('resultBody').innerHTML += row; // ❌ 루프마다 탐색 + 재렌더링
});
```
 
**해결**
 
부모 요소를 루프 밖에서 변수로 한 번만 참조하고, `appendChild()`로 완성된 요소를 한 번에 추가하여 불필요한 렌더링 비용을 줄였습니다.
 
```javascript
// ✅ 개선된 코드 (account-settings.html - 수정 후)
...
```java
// ❌ 문제가 된 코드 (Review.java - 어노테이션 적용 전)
// Review를 직렬화하면 → User를 직렬화 → User 안의 reviews를 직렬화 → 다시 Review... 무한 반복
// 결과: StackOverflowError 또는 HTTP 500 응답
 
@ManyToOne(fetch = FetchType.EAGER)
@JoinColumn(name = "user_id")
private User user; // ❌ @JsonIgnoreProperties 없음 → 순환 참조 발생
 
@ManyToOne(fetch = FetchType.LAZY)
@JoinColumn(name = "book_id")
private Book book; // ❌ creator(User) 필드가 다시 직렬화되며 순환
```
...
```java
// ❌ 문제가 된 코드 (BookController.java - 수정 전)
// Book 엔티티를 그대로 반환 → password 등 민감 정보 포함 가능
// creator 필드의 LAZY 로딩 → 세션 종료 후 LazyInitializationException 발생 위험
@GetMapping
public List<Book> getAllBooks() {
    return bookService.findAllBooks(); // ❌ 엔티티 직접 반환
}
 
@GetMapping("/{id}")
public Book getBook(@PathVariable Long id) {
    return bookService.findById(id); // ❌ DB 구조 그대로 노출
}
```
...
```java
// ✅ 개선된 코드 (BookController.java - 수정 후)
@GetMapping
public List<BookResponse> getAllBooks() {
    return bookService.findAllBooks()
            .stream()
            .map(BookResponse::new) // ✅ DTO로 변환 후 반환
            .toList();
}
 
// BookResponse.java - 필요한 필드만 담은 DTO
public class BookResponse {
    private final Long bookId;
    private final String title;
    private final String author;
    private final String creatorNickname; // ✅ creator 엔티티 전체 대신 닉네임만 노출
...
```java
// ❌ 문제가 된 상황 예시
// 케이스 1: 모두 EAGER → 리뷰 1건 조회할 때도 User, Book, Book의 creator까지 전부 JOIN
@ManyToOne(fetch = FetchType.EAGER)
private User user;
@ManyToOne(fetch = FetchType.EAGER)
private Book book; // ❌ book 조회 시 creator(User)까지 항상 조회 → 불필요한 쿼리 증가
 
// 케이스 2: 모두 LAZY → 트랜잭션 밖에서 user.getNickname() 호출 시
// org.hibernate.LazyInitializationException: could not initialize proxy 발생
@ManyToOne(fetch = FetchType.LAZY)
private User user; // ❌ 세션 종료 후 접근 시 예외 발생
```
...
```java
// ✅ 개선된 코드 (Review.java - 수정 후)
// User: 리뷰 조회 시 작성자 닉네임이 항상 필요하므로 EAGER로 즉시 로딩
@ManyToOne(fetch = FetchType.EAGER)
private User user;
 
// Book: DTO 변환 시 트랜잭션 안에서만 접근하므로 LAZY로 필요할 때만 로딩
@ManyToOne(fetch = FetchType.LAZY)
private Book book;
```
...
좋아요 추가(`POST`)와 취소(`DELETE`)를 별도 API로 분리하면, 클라이언트가 현재 좋아요 상태를 별도로 조회한 뒤 분기 처리해야 하므로 구조가 복잡해집니다. 또한 상태 불일치 시 중복 좋아요가 DB에 쌓일 위험이 있습니다.
 
```java
// ❌ 문제가 된 구조 (분리 API 방식)
// 클라이언트가 상태를 직접 판단해서 다른 API를 호출해야 함
POST   /api/hearts/{bookId}         // 좋아요 추가
DELETE /api/hearts/{bookId}         // 좋아요 취소
// → 클라이언트 코드가 복잡해지고, 동시 요청 시 중복 데이터 생성 가능
```
 
**해결**
 
단일 `POST /api/hearts/{bookId}` 엔드포인트에서 DB에 데이터가 존재하면 삭제, 없으면 생성하는 **토글 방식**으로 구현하여 API를 단순화하고 중복을 방지했습니다.
 
```java
// ✅ 개선된 코드 (HeartService.java - 토글 방식)
@Transactional
public boolean toggleHeart(Long userId, Long bookId) {
    ...
    if (heart.isPresent()) {
        heartRepository.delete(heart.get()); // ✅ 이미 있으면 삭제 (좋아요 취소)
        return false;
    } else {
        heartRepository.save(Heart.builder().user(user).book(book).build()); // ✅ 없으면 생성
        return true;
    }
    // 반환값(true/false)으로 클라이언트가 현재 상태를 바로 알 수 있음
}
```
